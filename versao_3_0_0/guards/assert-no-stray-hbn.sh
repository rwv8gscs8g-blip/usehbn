...
if [[ -z "$ALLOWLIST_FILE" ]]; then
    # Version-aware (terceira exuvia): o allowlist versionado mora na versao
    # ativa; fallback para a raiz do git (consumidores em project-mode).
    _AROOT="$(get_canonical_root 2>/dev/null || echo "")"
    _TOP="$(git rev-parse --show-toplevel 2>/dev/null || echo "")"
    if [[ -n "$_AROOT" && -f "${_AROOT}/.hbn/stray-allowlist" ]]; then
        ALLOWLIST_FILE="${_AROOT}/.hbn/stray-allowlist"
    elif [[ -n "$_TOP" && -f "${_TOP}/.hbn/stray-allowlist" ]]; then
        ALLOWLIST_FILE="${_TOP}/.hbn/stray-allowlist"
    fi
fi
is_allowlisted() {
    local d="$1" pat
