...
# requires:
#   files:
#     - path: schemas/dispatch.schema.json
#       install: copy
#       source: schemas/dispatch.schema.json
#   dirs: []
#   state_fields: []
#   guards: []
#   env: []
...
if ! git cat-file -e "$SCHEMA_REF" 2>/dev/null; then
    guard_fail "Schema obrigatório ausente no índice/HEAD: schemas/dispatch.schema.json (necessário somente quando há dispatch staged)."
    exit 1
fi
