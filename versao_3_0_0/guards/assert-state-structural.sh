...
# requires:
#   files:
#     - path: guards/data/auditor-families.txt
#       install: copy
#       source: guards/data/auditor-families.txt
...
# FIX terceira exuvia: readback e version-aware — o path listado e relativo a
# versao ativa, mas o blob mora em <prefixo><path>. Grava "path<TAB>ref" para
# o python nao remontar o ref na mao (bug: ':.hbn/readbacks/...' pos-flip).
while IFS= read -r rb_path; do
    [[ -z "$rb_path" ]] && continue
    rb_ref="$(blob_ref "$rb_path" || true)"
    [[ -n "$rb_ref" ]] || continue
    printf '%s\t%s\n' "$rb_path" "$rb_ref" >> "$READBACKS_FILE"
done < <(added_readbacks)
...
DIFF_BASE="$(hbn_effective_diff_base || true)"
if ! python3 - "$STATE_REF" "$OLD_STATE_REF" "$READBACKS_FILE" "$MAP_REF" "$RESULTS_FILE" "$REFS_FILE" "$DIFF_BASE" <<'PY'
import json
import os
import re
import subprocess
...
if os.path.exists(readbacks_file):
    with open(readbacks_file, encoding="utf-8") as f:
        for line in f:
            ...
            parts = line.split("\t", 1)
            path = parts[0]
            readbacks.append(path)
            if len(parts) == 2:
                readback_refs[path] = parts[1]
...
for rb_path in readbacks:
    rb_ref = readback_refs.get(rb_path) or (f"HEAD:{rb_path}" if use_head else f":{rb_path}")
    try:
