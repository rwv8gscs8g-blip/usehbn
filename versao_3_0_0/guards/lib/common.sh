...
guard_paths_to_version_paths() {
    # Jurisdicao (terceira exuvia): os guards da versao ativa governam SO a
    # versao ativa (+ raiz). Paths sob OUTRA versao_X_Y_Z/ (glacier/proposta)
    # sao filtrados aqui — a contencao de escrita neles e do G-HOT-WRITE
    # (que le o diff bruto de proposito) e do G-NO-PENDING-EXUVIA.
    # So filtra quando ha versao quente NUMERADA (prefixo nao-vazio): em
    # project-mode (active='.') o comportamento herdado permanece integral.
    local p prefix
    prefix="$(guard_active_version_prefix 2>/dev/null || echo "")"
    while IFS= read -r p; do
        [[ -z "$p" ]] && continue
        if [[ -n "$prefix" && "$p" =~ ^versao_[0-9]+_[0-9]+_[A-Za-z0-9]+/ && "$p" != "$prefix"* ]]; then
            continue
        fi
