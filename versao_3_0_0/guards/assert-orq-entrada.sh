...
if ! (
    cd "$REPO_ROOT"
        python3 - "$TOKEN_FP" "$ATTEST_REPO_PATH" "$EXPECTED_FILE" "$STATE_PATH" "$READBACK_PATH" "${HBN_ORQ_ENTRADA_GIT_REF:-}" "$ORQ_RANGE_MODE" <<'PY'
import hashlib
import json
import pathlib
import re
...
token_fp, attest_repo_path, expected_file, state_path, readback_path, git_ref, range_mode = sys.argv[1:8]
use_head = range_mode == "1"
errors = []
