...
unset HBN_GUARDS_BYPASS GLASSWING_BYPASS HBN_DIFF_BASE HBN_CI CI GITHUB_ACTIONS 2>/dev/null || true
...
make_num_repo() { # $1 = conteúdo inline de escrita_paralela, ex: "[alpha-1, beta-1]"
    ...
    (
        ...
        git config user.email "tests@hbn.local"
        git config user.name "hbn-guard-tests"
        mkdir -p .hbn/relay .hbn/models .hbn/messages .hbn/proposals reports docs/prompts docs
        echo "." > .hbn/active-version
        printf '{"apelido":"alpha-1","familia":"vendora","fornecedor":"VendorA"}\n' > .hbn/models/alpha-1.json
        printf '{"apelido":"beta-1","familia":"vendorb","fornecedor":"VendorB"}\n' > .hbn/models/beta-1.json
...
p = Path(".hbn/relay/STATE.md")
s = p.read_text()
s = s.replace("  auditores: [beta-1]\n  escrita_paralela: [alpha-1, beta-1]\n", "  auditores:\n    - beta-1\n  escrita_paralela:\n    - alpha-1\n    - beta-1\n")
p.write_text(s)
PY
    echo "ideia" > .hbn/proposals/20260610-101010-alpha-1-bloco.md
    echo "| 20260610-101010-alpha-1-bloco | .hbn/proposals/20260610-101010-alpha-1-bloco.md | proposal | quente | — | 2026-06-10T10:10:10-03:00 |" >> REGISTRY.md
    git add -A
) >/dev/null 2>&1
check "num: atribuicao YAML em bloco passa sem vacuo" pass "$(run_num "$d")"
rm -rf "$d"
...
check "arvore: fixture registry-skeleton sha256 byte-a-byte" pass \
    "$( [[ "$(sha256_file "$REPO_ROOT/guards/fixtures/registry-skeleton.md")" == "8a40bf06701b5d5ce0f45e0456b8b18d7008fdfd74e6421d7f0b4fdca538ede9" ]]; echo $? )"

make_arvore_genesis_repo() {
    local variant="$1" d; d="$(mktemp -d)"
    (
        cd "$d"
        git init -q
        git config user.email "tests@hbn.local"
        ...
        cat > .hbn/relay/STATE.md <<'EOF'
---
proxima_acao: "testar genese do REGISTRY"
---
EOF
        cp "$REPO_ROOT/guards/fixtures/registry-skeleton.md" REGISTRY.md
        case "$variant" in
            good)
                cat >> REGISTRY.md <<'EOF'
| 20260704-101010-codex-a | docs/a.md | doc | frio | fronteira | — | 2026-07-04T10:10:10-03:00 |
| 20260704-101011-codex-b | docs/b.md | doc | frio | fronteira | — | 2026-07-04T10:10:11-03:00 |
| 20260704-101012-codex-c | docs/c.md | doc | frio | fronteira | — | 2026-07-04T10:10:12-03:00 |
EOF
                ;;
            bad-arvore)
                cat >> REGISTRY.md <<'EOF'
| 20260704-101010-codex-a | docs/a.md | doc | frio | experimental | — | 2026-07-04T10:10:10-03:00 |
EOF
                ;;
        esac
...
d="$(make_arvore_genesis_repo good)"
check "arvore: genese valida trata delta como arquivo inteiro" pass "$(run_arvore "$d")"
rm -rf "$d"

d="$(make_arvore_genesis_repo bad-arvore)"
check "arvore: genese com arvore invalida bloqueia" block "$(run_arvore "$d")"
rm -rf "$d"
...
run_exc_ci() { ( cd "$1" && HBN_CI=1 HBN_DIFF_BASE="$2" bash "$GUARDS_DIR/assert-exception-traceable.sh" >/dev/null 2>&1 ); echo $?; }
SINAL_OK='  - "🔴 EXCEÇÃO F-01 ATIVA — PROPOSED_UNTIL_CROSS_AUDIT"'
SINAL_SEM='  - "🟢 tudo normal"'

d="$(make_repo)"
check "exc: genese sem STATE -> NO-OP" pass "$(run_exc "$d")"
rm -rf "$d"

d="$(make_exc_repo "mesmo-1" "mesmo-1" y "$SINAL_OK")"
( cd "$d" && git rm -q .hbn/relay/STATE.md ) >/dev/null 2>&1
check "exc: remocao de STATE existente -> BLOCK anti-desarme" block "$(run_exc "$d")"
rm -rf "$d"
...
d="$(make_repo)"
check "fam-run: genese sem STATE -> NO-OP" pass "$( ( cd "$d" && bash "$GUARDS_DIR/assert-role-family.sh" >/dev/null 2>&1 ); echo $? )"
rm -rf "$d"
d="$(make_fam_state_repo "fulano-1" "[beltrano-2]")"
( cd "$d" && git rm -q .hbn/relay/STATE.md ) >/dev/null 2>&1
check "fam-run: remocao de STATE existente -> BLOCK anti-desarme" block "$( ( cd "$d" && bash "$GUARDS_DIR/assert-role-family.sh" >/dev/null 2>&1 ); echo $? )"
rm -rf "$d"
...
check "fam-run: auditores em bloco YAML passam"          pass  "$( ( cd "$d" && bash "$GUARDS_DIR/assert-role-family.sh" >/dev/null 2>&1 ); echo $? )"
rm -rf "$d"
d="$(make_fam_state_repo "fulano-1" "[beltrano-2]")"
(
    cd "$d"
    printf -- '---\nproxima_acao: sem atribuicao\n---\n' > .hbn/relay/STATE.md
    git add .hbn/relay/STATE.md
) >/dev/null 2>&1
check "fam-run: sem atribuicao nao passa em vacuo"       block "$( ( cd "$d" && bash "$GUARDS_DIR/assert-role-family.sh" >/dev/null 2>&1 ); echo $? )"
rm -rf "$d"
...
d="$(make_repo)"
check "dsp-fmt: sem dispatch staged nao exige schema" pass "$(run_dsp_fmt "$d")"
rm -rf "$d"
...
(
    ...
    cat > .hbn/dispatch/0001-sem-schema.md <<'EOF'
---
schema_version: dispatch.v1
dispatch_id: 0001-sem-schema
path: .hbn/dispatch/0001-sem-schema.md
readback_id: 0001-sem-schema
token_fp: 34a7f2f9
...
executar
EOF
    git add .hbn/dispatch/0001-sem-schema.md
) >/dev/null 2>&1
check "dsp-fmt: dispatch staged sem schema segue BLOCK" block "$(run_dsp_fmt "$d")"
rm -rf "$d"
...
d="$(make_repo)"
check "know: genese sem INDEX -> NO-OP" pass "$(run_know "$d")"
rm -rf "$d"
...
d="$(make_know_repo)"
( cd "$d" && git rm -q .hbn/knowledge/INDEX.md ) >/dev/null 2>&1
check "know: remocao de INDEX existente -> BLOCK anti-desarme" block "$(run_know "$d")"
rm -rf "$d"
...
make_frontdoor_repo() {
    ...
    (
        ...
        echo "# Knowledge Index" > .hbn/knowledge/INDEX.md
        echo ".hbn/relay/STATE.md" > core/read-list-canonica.txt
        write_frontdoor_valid
        git add core/role-cards.md core/read-list-canonica.txt .hbn/relay/STATE.md \
            .hbn/knowledge/INDEX.md
...
d="$(make_repo)"
check "frontdoor: genese sem role-cards -> NO-OP" pass "$(run_frontdoor "$d")"
rm -rf "$d"
...
check "frontdoor: remocao de role-cards existente -> BLOCK anti-desarme" block "$(run_frontdoor "$d")"
rm -rf "$d"
...
(
    cd "$d"
    awk '1; /^5\. / { print "6. `extra/read-list-estourada.md`"; print "7. `extra/read-list-estourada-2.md`" }' core/role-cards.md > core/role-cards.tmp
...
(
    cd "$d"
    sed -i.bak 's#^4\. `core/read-list-canonica.txt`#4. `core/inexistente-na-read-list.md`#' core/role-cards.md
    rm -f core/role-cards.md.bak
    git add core/role-cards.md
) >/dev/null 2>&1
...
check "orq-ref CI: selagem same-fp passa no range" pass "$( ( cd "$d" && HBN_CI=1 HBN_DIFF_BASE="$base" bash "$GUARDS_DIR/assert-orq-entrada-ref.sh" >/dev/null 2>&1 ); echo $? )"
rm -rf "$d"
...
check "orq-ref CI: fp trocado bloqueia no range" block "$( ( cd "$d" && HBN_CI=1 HBN_DIFF_BASE="$base" bash "$GUARDS_DIR/assert-orq-entrada-ref.sh" >/dev/null 2>&1 ); echo $? )"
rm -rf "$d"
...
d="$(make_repo)"
check "ci-battery: genese sem workflow -> NO-OP" pass "$(run_ci_battery_guard "$d")"
rm -rf "$d"
...
d="$(make_ci_battery_repo good)"
( cd "$d" && git commit -qm init && git rm -q .github/workflows/hbn-shield.yml ) >/dev/null 2>&1
check "ci-battery: remocao de workflow existente -> BLOCK anti-desarme" block "$(run_ci_battery_guard "$d")"
rm -rf "$d"
...
# --- Fase 1 Exuvia: requires/MANIFEST, instalador, matriz e genese ----------
echo "== fase 1 exuvia: manifest/install/profile/genese =="
...
make_manifest_repo() {
    ...
    (
        ...
        mkdir -p .hbn
        echo "." > .hbn/active-version
        cp -R "$REPO_ROOT/guards" guards
        rm -rf guards/tests
        bash guards/generate-manifest.sh > guards/MANIFEST.yaml
        git add .hbn/active-version guards
        git commit -qm manifest
    ) >/dev/null 2>&1
    echo "$d"
}
d="$(make_manifest_repo)"
check "manifest: gerado bate com blocos requires" pass "$( ( cd "$d" && bash guards/assert-manifest-current.sh >/dev/null 2>&1 ); echo $? )"
(
    cd "$d"
    printf '\n# drift\n' >> guards/MANIFEST.yaml
    git add guards/MANIFEST.yaml
) >/dev/null 2>&1
check "manifest: drift manual bloqueia" block "$( ( cd "$d" && bash guards/assert-manifest-current.sh >/dev/null 2>&1 ); echo $? )"
rm -rf "$d"
...
d="$(make_install_target)"
( cd "$REPO_ROOT" && scripts/hbn-install-guard --target "$d" --path REGISTRY.md --install skeleton >/dev/null 2>&1 )
check "install: skeleton REGISTRY byte-a-byte" pass \
    "$( cmp -s "$REPO_ROOT/guards/fixtures/registry-skeleton.md" "$d/REGISTRY.md"; echo $? )"
( cd "$REPO_ROOT" && scripts/hbn-install-guard --target "$d" --path .hbn/models --install skeleton >/dev/null 2>&1 )
check "install: skeleton models por atribuicao" pass \
    "$( [[ -f "$d/.hbn/models/codex.json" && -f "$d/.hbn/models/grok.json" && -f "$d/.hbn/models/antigravity.json" ]]; echo $? )"
check "install: refuse instrutivo bloqueia" block \
    "$( ( cd "$REPO_ROOT" && scripts/hbn-install-guard --target "$d" --path .github/workflows/hbn-shield.yml --install refuse >/dev/null 2>&1 ); echo $? )"
rm -rf "$d"

write_profile_matrix() { # <file> [skip_guard]
    local out="$1" skip="${2:-}" g
    {
        echo "guards_projeto:"
        while IFS= read -r g; do
            [[ "$g" == "$skip" ]] && continue
            echo "  - $g"
        done < <(find "$GUARDS_DIR" -maxdepth 1 -type f \( -name 'assert-*.sh' -o -name 'forbid-*.sh' -o -name 'freeze-gate.sh' -o -name 'validate-dispatch.sh' \) -exec basename {} .sh \; | sort)
        echo "guards_orquestracao: []"
        echo "guards_condicionais: []"
        echo "guards_commit_msg: []"
        echo "guards_a_instalar: []"
        echo "exclusoes_autorizadas: []"
    } > "$out"
}
d="$(make_repo)"
check "profile: genoma sem CONSUMER-PROFILE no-op" pass \
    "$( ( cd "$d" && bash "$GUARDS_DIR/assert-profile-authorized.sh" >/dev/null 2>&1 ); echo $? )"
rm -rf "$d"

d="$(make_repo)"
( cd "$d" && HBN_CONSUMER_PROFILE="$d/CONSUMER-PROFILE.md" bash "$GUARDS_DIR/assert-profile-authorized.sh" >/dev/null 2>&1 )
check "profile: HBN_CONSUMER_PROFILE global sem consumidor versionado -> NO-OP" pass "$?"
rm -rf "$d"

d="$(make_repo)"
( cd "$d" && mkdir -p .usehbn-snapshot/guards ) >/dev/null 2>&1
check "profile: .usehbn-snapshot orfao no filesystem -> NO-OP" pass \
    "$( ( cd "$d" && bash "$GUARDS_DIR/assert-profile-authorized.sh" >/dev/null 2>&1 ); echo $? )"
rm -rf "$d"

d="$(make_repo)"
( cd "$d" && mkdir -p .usehbn-snapshot/guards && echo "marker" > .usehbn-snapshot/guards/.keep && git add .usehbn-snapshot/guards/.keep ) >/dev/null 2>&1
check "profile: marcador versionado sem perfil bloqueia" block \
    "$( ( cd "$d" && bash "$GUARDS_DIR/assert-profile-authorized.sh" >/dev/null 2>&1 ); echo $? )"
rm -rf "$d"

d="$(make_repo)"
write_profile_matrix "$d/CONSUMER-PROFILE.md"
check "profile: matriz total cobre todos os guards fisicos" pass \
    "$( ( cd "$d" && git add CONSUMER-PROFILE.md && HBN_PROFILE_SNAPSHOT_GUARDS_DIR="$GUARDS_DIR" bash "$GUARDS_DIR/assert-profile-authorized.sh" >/dev/null 2>&1 ); echo $? )"
write_profile_matrix "$d/CONSUMER-PROFILE.md" "assert-frontdoor"
check "profile: guard fisico omitido bloqueia" block \
    "$( ( cd "$d" && git add CONSUMER-PROFILE.md && HBN_PROFILE_SNAPSHOT_GUARDS_DIR="$GUARDS_DIR" bash "$GUARDS_DIR/assert-profile-authorized.sh" >/dev/null 2>&1 ); echo $? )"
rm -rf "$d"

make_profile_consumer_repo() {
    local d; d="$(make_repo)"
    (
        cd "$d"
        mkdir -p .usehbn-snapshot/guards
        echo "marker" > .usehbn-snapshot/guards/.keep
        write_profile_matrix CONSUMER-PROFILE.md
        git add CONSUMER-PROFILE.md .usehbn-snapshot/guards/.keep
        git commit -qm consumer
    ) >/dev/null 2>&1
    echo "$d"
}
d="$(make_profile_consumer_repo)"
( cd "$d" && git rm -rq .usehbn-snapshot ) >/dev/null 2>&1
check "profile: consumidor tenta apagar .usehbn-snapshot -> BLOCK anti-desarme" block \
    "$( ( cd "$d" && HBN_PROFILE_SNAPSHOT_GUARDS_DIR="$GUARDS_DIR" bash "$GUARDS_DIR/assert-profile-authorized.sh" >/dev/null 2>&1 ); echo $? )"
rm -rf "$d"

d="$(make_profile_consumer_repo)"
( cd "$d" && git rm -q CONSUMER-PROFILE.md ) >/dev/null 2>&1
check "profile: consumidor tenta apagar CONSUMER-PROFILE -> BLOCK anti-desarme" block \
    "$( ( cd "$d" && HBN_PROFILE_SNAPSHOT_GUARDS_DIR="$GUARDS_DIR" bash "$GUARDS_DIR/assert-profile-authorized.sh" >/dev/null 2>&1 ); echo $? )"
rm -rf "$d"

# --- Anti-desarme torto: HBN_DIFF_BASE local, rename, symlink, active-version --
echo "== anti-desarme torto (contexto CI-real + indice + dirs) =="

make_antidisarm_dep_repo() { # <state|knowledge|frontdoor|ci|profile-root|profile-marker>
    local dep="$1" d
    case "$dep" in
        state)
            d="$(make_fam_state_repo "fulano-1" "[beltrano-2]")"
        ...
        ci)
            d="$(make_ci_battery_repo good)"
            ( cd "$d" && git commit -qm ci-battery ) >/dev/null 2>&1
            ;;
        profile-root|profile-marker)
            d="$(make_profile_consumer_repo)"
            ;;
        *)
            return 1
...
antidisarm_dep_path() { # <dep>
    case "$1" in
        state) printf '.hbn/relay/STATE.md\n' ;;
        knowledge) printf '.hbn/knowledge/INDEX.md\n' ;;
        frontdoor) printf 'core/role-cards.md\n' ;;
        ci) printf '.github/workflows/hbn-shield.yml\n' ;;
        profile-root) printf 'CONSUMER-PROFILE.md\n' ;;
        profile-marker) printf '.usehbn-snapshot\n' ;;
        *) return 1 ;;
    esac
}

run_antidisarm_dep_guard() { # <dep> <repo> [extra-env]
    local dep="$1" repo="$2"
    case "$dep" in
        state)
            ( cd "$repo" && bash "$GUARDS_DIR/assert-role-family.sh" >/dev/null 2>&1 )
            ;;
        knowledge)
            ( cd "$repo" && bash "$GUARDS_DIR/assert-knowledge-index.sh" >/dev/null 2>&1 )
            ;;
        frontdoor)
            ( cd "$repo" && bash "$GUARDS_DIR/assert-frontdoor.sh" >/dev/null 2>&1 )
            ;;
        ci)
            ( cd "$repo" && bash "$GUARDS_DIR/assert-ci-battery.sh" >/dev/null 2>&1 )
            ;;
        profile-root|profile-marker)
            ( cd "$repo" && HBN_PROFILE_SNAPSHOT_GUARDS_DIR="$GUARDS_DIR" bash "$GUARDS_DIR/assert-profile-authorized.sh" >/dev/null 2>&1 )
            ;;
        *)
            return 2
            ;;
...
run_active_version_integrity_guard() { # <repo>
    ( cd "$1" && bash "$GUARDS_DIR/assert-active-version-integrity.sh" >/dev/null 2>&1 )
    echo $?
}

stage_antidisarm_remove() { # <repo> <path>
    local repo="$1" path="$2"
    ( cd "$repo" && git rm -rq -- "$path" ) >/dev/null 2>&1
}

stage_antidisarm_rename() { # <repo> <path>
    local repo="$1" path="$2" dest
    dest="${path}.renomeado"
    [[ "$path" == ".usehbn-snapshot" ]] && dest=".usehbn-snapshot-renomeado"
    ( cd "$repo" && git mv -- "$path" "$dest" ) >/dev/null 2>&1
}

stage_antidisarm_symlink() { # <repo> <path>
    ...
    (
        ...
        ln -s anti-desarme-target "$path"
        git add -- "$path"
    ) >/dev/null 2>&1
}

stage_active_version_skeleton() { # <repo>
    local repo="$1"
    (
        cd "$repo"
        mkdir -p versao_1_0_0
        echo "skeleton" > versao_1_0_0/.keep
        echo "versao_1_0_0" > .hbn/active-version
        git add .hbn/active-version versao_1_0_0/.keep
    ) >/dev/null 2>&1
}

check_antidisarm_case() { # <dep> <variant>
    local dep="$1" variant="$2" repo path base rc
    repo="$(make_antidisarm_dep_repo "$dep")"
    path="$(antidisarm_dep_path "$dep")"
    base="$(cd "$repo" && git rev-parse HEAD)"
    case "$variant" in
        poison-remove)
            stage_antidisarm_remove "$repo" "$path"
            rc="$( ( unset CI GITHUB_ACTIONS HBN_CI; export HBN_DIFF_BASE="$base"; run_antidisarm_dep_guard "$dep" "$repo" ); echo $? )"
            ;;
        ci-true-remove)
            stage_antidisarm_remove "$repo" "$path"
            rc="$( ( unset GITHUB_ACTIONS HBN_CI; export CI=true HBN_DIFF_BASE="$base"; run_antidisarm_dep_guard "$dep" "$repo" ); echo $? )"
            ;;
        github-actions-remove)
            stage_antidisarm_remove "$repo" "$path"
            rc="$( ( unset CI HBN_CI; export GITHUB_ACTIONS=true HBN_DIFF_BASE="$base"; run_antidisarm_dep_guard "$dep" "$repo" ); echo $? )"
            ;;
        hbn-ci-remove)
            stage_antidisarm_remove "$repo" "$path"
            rc="$( ( unset CI GITHUB_ACTIONS; export HBN_CI=1 HBN_DIFF_BASE="$base"; run_antidisarm_dep_guard "$dep" "$repo" ); echo $? )"
            ;;
        rename)
            stage_antidisarm_rename "$repo" "$path"
            rc="$(run_antidisarm_dep_guard "$dep" "$repo"; echo $?)"
            ;;
        symlink)
            stage_antidisarm_symlink "$repo" "$path"
            rc="$(run_antidisarm_dep_guard "$dep" "$repo"; echo $?)"
    ...
    check "anti-desarme ${dep}: ${variant} -> BLOCK" block "$rc"
    rm -rf "$repo"
}

check_antidisarm_active_version_case() { # <dep>
    local dep="$1" repo rc
    repo="$(make_antidisarm_dep_repo "$dep")"
    stage_active_version_skeleton "$repo"
    rc="$(run_antidisarm_dep_guard "$dep" "$repo"; echo $?)"
    check "anti-desarme ${dep}: active-version -> esqueleto sem deps -> BLOCK" block "$rc"
    rm -rf "$repo"
}

check_antidisarm_genesis_case() { # <dep>
    local dep="$1" repo rc
    repo="$(make_repo)"
    rc="$(run_antidisarm_dep_guard "$dep" "$repo"; echo $?)"
    check "anti-desarme ${dep}: genese real nunca carregou dep -> NOOP/PASS" pass "$rc"
    rm -rf "$repo"
}

for dep in state knowledge frontdoor ci profile-root profile-marker; do
    check_antidisarm_case "$dep" poison-remove
    check_antidisarm_case "$dep" ci-true-remove
    check_antidisarm_case "$dep" github-actions-remove
    check_antidisarm_case "$dep" hbn-ci-remove
    check_antidisarm_case "$dep" rename
    check_antidisarm_case "$dep" symlink
    check_antidisarm_active_version_case "$dep"
    check_antidisarm_genesis_case "$dep"
done

d="$(make_antidisarm_dep_repo state)"
stage_active_version_skeleton "$d"
check "active-version-integrity: esqueleto sem STATE protegido -> BLOCK" block "$(run_active_version_integrity_guard "$d")"
rm -rf "$d"

make_versioned_antidisarm_state_repo() {
    local d; d="$(mktemp -d)"
    (
        cd "$d"
        git init -q
        git config user.email "tests@hbn.local"
        git config user.name "hbn-guard-tests"
        mkdir -p .hbn versao_1_0_0/.hbn/relay
        echo "versao_1_0_0" > .hbn/active-version
        printf -- '---\natribuicao:\n  implementador: fulano-1\n  auditores: [beltrano-2]\n---\n' > versao_1_0_0/.hbn/relay/STATE.md
        git add -A
        git commit -qm versioned-state
    ) >/dev/null 2>&1
    echo "$d"
}

d="$(make_versioned_antidisarm_state_repo)"
base="$(cd "$d" && git rev-parse HEAD)"
( cd "$d" && git rm -q versao_1_0_0/.hbn/relay/STATE.md ) >/dev/null 2>&1
check "anti-desarme active-version != .: STATE removido com HBN_DIFF_BASE local -> BLOCK" block \
    "$( ( cd "$d" && unset CI GITHUB_ACTIONS HBN_CI; HBN_DIFF_BASE="$base" bash "$GUARDS_DIR/assert-role-family.sh" >/dev/null 2>&1 ); echo $? )"
rm -rf "$d"

make_versioned_antidisarm_profile_repo() {
    local d; d="$(mktemp -d)"
    (
        cd "$d"
        git init -q
        git config user.email "tests@hbn.local"
        git config user.name "hbn-guard-tests"
        mkdir -p .hbn versao_1_0_0/.usehbn-snapshot/guards
        echo "versao_1_0_0" > .hbn/active-version
        echo "marker" > versao_1_0_0/.usehbn-snapshot/guards/.keep
        write_profile_matrix versao_1_0_0/CONSUMER-PROFILE.md
        git add -A
        git commit -qm versioned-profile
    ) >/dev/null 2>&1
    echo "$d"
}

d="$(make_versioned_antidisarm_profile_repo)"
( cd "$d" && git mv versao_1_0_0/.usehbn-snapshot versao_1_0_0/.usehbn-snapshot-renomeado ) >/dev/null 2>&1
check "anti-desarme active-version != .: marcador renomeado -> BLOCK" block \
    "$( ( cd "$d" && HBN_PROFILE_SNAPSHOT_GUARDS_DIR="$GUARDS_DIR" bash "$GUARDS_DIR/assert-profile-authorized.sh" >/dev/null 2>&1 ); echo $? )"
rm -rf "$d"
...
make_integrated_genesis_repo() {
    ...
    (
        ...
        git config user.email "tests@hbn.local"
        git config user.name "hbn-guard-tests"
        mkdir -p .hbn/relay .hbn/readbacks .hbn/models .hbn/knowledge core guards/data docs
        echo "." > .hbn/active-version
        pwd -P > .hbn/canonical-root
        cat > guards/data/auditor-families.txt <<'EOF'
codex OpenAI
...
---
state_version: 1
proxima_acao: "concluir genese"
readback_ativo: ".hbn/readbacks/0001-genesis.json"
handoff_mais_recente: ".hbn/messages/20260704-101000-codex-handoff.md"
...
proximo_ponto:
  ...
  status: pendente
sinais_abertos: []
ultima_atualizacao: "2026-07-04T10:10:00-03:00"
...
---
EOF
        cat > .hbn/readbacks/0001-genesis.json <<'EOF'
{"readback_id":"0001-genesis","execution_id":"genesis","agent_id":"opus","track":"safe_track","human_status":"confirmed","hearback_status":"confirmed","read_list_rite":"genese","authorization":{"human":"Mauricio","evidence":"teste"},"scope":{"files_allowed":["**"],"files_forbidden":[]}}
EOF
        printf '{"apelido":"codex","familia":"openai","fornecedor":"openai"}\n' > .hbn/models/codex.json
        printf '{"apelido":"grok","familia":"xai","fornecedor":"xai"}\n' > .hbn/models/grok.json
        printf '{"apelido":"antigravity","familia":"google","fornecedor":"google"}\n' > .hbn/models/antigravity.json
        cp "$REPO_ROOT/guards/fixtures/role-cards-consumer.md" core/role-cards.md
        printf '.hbn/relay/STATE.md\n.hbn/readbacks/0001-genesis.json\ncore/role-cards.md\n.hbn/knowledge/INDEX.md\n' > core/read-list-canonica.txt
        printf '# Knowledge Index\n' > .hbn/knowledge/INDEX.md
        cp "$REPO_ROOT/guards/fixtures/registry-skeleton.md" REGISTRY.md
        cat >> REGISTRY.md <<'EOF'
| 20260704-101000-codex-readback | .hbn/readbacks/0001-genesis.json | readback | frio | fronteira | — | 2026-07-04T10:10:00-03:00 |
| 20260704-101001-codex-role-cards | core/role-cards.md | core-spec | frio | fronteira | — | 2026-07-04T10:10:01-03:00 |
| 20260704-101002-codex-model | .hbn/models/codex.json | model-profile | frio | fronteira | — | 2026-07-04T10:10:02-03:00 |
| 20260704-101003-codex-model | .hbn/models/grok.json | model-profile | frio | fronteira | — | 2026-07-04T10:10:03-03:00 |
| 20260704-101004-codex-model | .hbn/models/antigravity.json | model-profile | frio | fronteira | — | 2026-07-04T10:10:04-03:00 |
EOF
        git add -A
    ) >/dev/null 2>&1
    echo "$d"
...
run_genesis_31() {
    local d="$1" g rc=0
    local guards=(
        assert-active-version-integrity
        assert-canonical-root forbid-tmp-worktree forbid-env-files forbid-legacy-paths
        assert-scratch-lock assert-scratch-symlink assert-scratch-ignore assert-zona-livre
        assert-scope-lock assert-hearback-integrity assert-no-stray-hbn assert-self-path
        assert-report-fresh assert-readlist-rite assert-knowledge-index assert-orq-entrada
        assert-orq-entrada-ref assert-registry-line assert-pointer-honest assert-arvore-label
        assert-parallel-id validate-dispatch assert-dispatch-integrity assert-auditor-id
        assert-audit-diversity assert-quorum-selagem assert-frontdoor assert-role-family
        assert-exception-traceable assert-next-checkpoint assert-copy-block
    )
    for g in "${guards[@]}"; do
        ( cd "$d" && HBN_SCAN_ROOT="$d" bash "$GUARDS_DIR/${g}.sh" >/dev/null 2>&1 ) || rc=1
    done
    echo "$rc"
}
d="$(make_integrated_genesis_repo)"
check "genese integrada: consumidor novo com 32 guards pre-commit verdes" pass "$(run_genesis_31 "$d")"
rm -rf "$d"
...
# quente passa; raiz fora da allowlist bloqueia; allowlist passa; flip de
# ponteiro sem autorizacao bloqueia; bypass e ignorado; auto-protecao bloqueia
# alteracao do proprio guard; exuvia AUTORIZADA (STATE staged + readback
# confirmed) passa; autorizacao pendente bloqueia.
echo "== assert-only-hot-version-writable (G-HOT-WRITE) =="
make_hotwrite_repo() {
    local d
    d="$(mktemp -d)"
    (
        cd "$d"
        git init -q
        git config user.email "t@hbn.local"
        git config user.name "hbn-test"
        mkdir -p .hbn/relay versao_3_0_0/guards/lib versao_3_0_0/.hbn/relay versao_3_0_0/.hbn/readbacks
        printf 'versao_3_0_0\n' > .hbn/active-version
        printf '*.log\n' > .gitignore
        printf -- '---\ntipo: relay-legado\n---\n' > .hbn/relay/STATE.md
        cp "$GUARDS_DIR/assert-only-hot-version-writable.sh" versao_3_0_0/guards/
        cp "$GUARDS_DIR/lib/common.sh" versao_3_0_0/guards/lib/
        printf '# legado na raiz\n' > legado.md
        printf '# base viva\n' > versao_3_0_0/base.md
        printf -- '---\ntipo: state\nversao: versao_3_0_0\n---\n# STATE\n' > versao_3_0_0/.hbn/relay/STATE.md
        git add -A
        git commit -qm base
    ) >/dev/null 2>&1
    echo "$d"
}
run_hotwrite() { # <repo> [env...]
    local d="$1"; shift
    ( cd "$d" && env "$@" bash "$GUARDS_DIR/assert-only-hot-version-writable.sh" >/dev/null 2>&1 )
    echo $?
}
stage_exuvia_to_v4() { # <repo> <auth-json-body|"">  — flip + pasta nova + STATE com marcador
    (
        cd "$1"
        mkdir -p versao_4_0_0/.hbn/relay versao_4_0_0/.hbn/readbacks versao_4_0_0/guards
        cp -R versao_3_0_0/guards/. versao_4_0_0/guards/
        printf 'versao_4_0_0\n' > .hbn/active-version
        cat > versao_4_0_0/.hbn/relay/STATE.md <<'EOF'
---
tipo: state
versao: versao_4_0_0
transicao: hot-write-exuvia
hearback_ref: .hbn/readbacks/0100-exuvia.json
---
# STATE v4
EOF
        if [[ -n "$2" ]]; then
            printf '%s\n' "$2" > versao_4_0_0/.hbn/readbacks/0100-exuvia.json
        fi
        git add -A
    ) >/dev/null 2>&1
}
...
( cd "$d" && printf 'mais\n' >> versao_3_0_0/base.md && git add versao_3_0_0/base.md ) >/dev/null 2>&1
check "hot-write: escrita dentro da versao quente passa" pass "$(run_hotwrite "$d")"
rm -rf "$d"

d="$(make_hotwrite_repo)"
( cd "$d" && printf 'intruso\n' > rogue.md && git add rogue.md ) >/dev/null 2>&1
check "hot-write: arquivo novo na raiz fora da allowlist BLOQUEIA" block "$(run_hotwrite "$d")"
check "hot-write: bypass HBN_GUARDS_BYPASS=1 e IGNORADO (continua bloqueando)" block "$(run_hotwrite "$d" HBN_GUARDS_BYPASS=1 GLASSWING_BYPASS=1)"
rm -rf "$d"

d="$(make_hotwrite_repo)"
( cd "$d" && printf '*.tmp\n' >> .gitignore && printf 'nota\n' >> .hbn/relay/STATE.md && git add .gitignore .hbn/relay/STATE.md ) >/dev/null 2>&1
check "hot-write: allowlist de raiz (.gitignore + relay STATE) passa" pass "$(run_hotwrite "$d")"
rm -rf "$d"

d="$(make_hotwrite_repo)"
( cd "$d" && printf 'versao_4_0_0\n' > .hbn/active-version && git add .hbn/active-version ) >/dev/null 2>&1
check "hot-write: flip do ponteiro SEM autorizacao de transicao BLOQUEIA" block "$(run_hotwrite "$d")"
rm -rf "$d"

d="$(make_hotwrite_repo)"
( cd "$d" && printf '# sabotagem\n' >> versao_3_0_0/guards/assert-only-hot-version-writable.sh \
    && git add versao_3_0_0/guards/assert-only-hot-version-writable.sh ) >/dev/null 2>&1
check "hot-write: AUTO-PROTECAO — alterar o proprio guard sem autorizacao BLOQUEIA" block "$(run_hotwrite "$d")"
rm -rf "$d"

d="$(make_hotwrite_repo)"
stage_exuvia_to_v4 "$d" '{"human_status":"confirmed","excecoes_cobertas":[{"tipo":"hot-write-exuvia","from":"versao_3_0_0","to":"versao_4_0_0"}]}'
check "hot-write: EXUVIA autorizada (STATE staged + readback confirmed) passa" pass "$(run_hotwrite "$d")"
rm -rf "$d"

d="$(make_hotwrite_repo)"
stage_exuvia_to_v4 "$d" '{"human_status":"pendente","excecoes_cobertas":[{"tipo":"hot-write-exuvia","from":"versao_3_0_0","to":"versao_4_0_0"}]}'
check "hot-write: exuvia com autorizacao PENDENTE bloqueia" block "$(run_hotwrite "$d")"
rm -rf "$d"

d="$(make_hotwrite_repo)"
stage_exuvia_to_v4 "$d" ""
check "hot-write: exuvia com hearback_ref dangling bloqueia" block "$(run_hotwrite "$d")"
rm -rf "$d"

d="$(make_hotwrite_repo)"
stage_exuvia_to_v4 "$d" '{"human_status":"confirmed","excecoes_cobertas":[{"tipo":"hot-write-exuvia","from":"versao_3_0_0","to":"versao_4_0_0"}]}'
( cd "$d" && mkdir -p versao_0_9_9 && git mv legado.md versao_0_9_9/legado.md ) >/dev/null 2>&1
check "hot-write: exuvia autorizada aceita RENAME raiz->glacier (git mv)" pass "$(run_hotwrite "$d")"
rm -rf "$d"
...
# TAREFA 5: nunca mais um "proposto" pendurado. Casos: sem pendencia passa;
# dev normal com versao proposta pendurada bloqueia; ativacao (flip+ativo)
# resolve; congelamento da proposta resolve; versao ATIVA proposta bloqueia.
echo "== assert-no-pending-exuvia (G-NO-PENDING-EXUVIA) =="
make_pending_repo() { # <status-boot-v3> [status-boot-v4]
    local d st3="$1" st4="${2:-}"
    d="$(mktemp -d)"
    (
        cd "$d"
        ...
        git config user.email "t@hbn.local"
        git config user.name "hbn-test"
        mkdir -p .hbn versao_3_0_0
        printf 'versao_3_0_0\n' > .hbn/active-version
        printf -- '---\ntitulo: boot v3\nstatus: %s\n---\n# BOOT\n' "$st3" > versao_3_0_0/BOOT.md
        printf '# base\n' > versao_3_0_0/base.md
        if [[ -n "$st4" ]]; then
            mkdir -p versao_4_0_0
            printf -- '---\ntitulo: boot v4\nstatus: %s\n---\n# BOOT\n' "$st4" > versao_4_0_0/BOOT.md
        fi
        git add -A
        git commit -qm base
    ) >/dev/null 2>&1
...
run_pending() { ( cd "$1" && bash "$GUARDS_DIR/assert-no-pending-exuvia.sh" >/dev/null 2>&1 ); echo $?; }

d="$(make_pending_repo ativo)"
( cd "$d" && printf 'dev\n' >> versao_3_0_0/base.md && git add versao_3_0_0/base.md ) >/dev/null 2>&1
check "no-pending: sem versao proposta, dev normal passa" pass "$(run_pending "$d")"
rm -rf "$d"

d="$(make_pending_repo ativo proposto)"
( cd "$d" && printf 'dev\n' >> versao_3_0_0/base.md && git add versao_3_0_0/base.md ) >/dev/null 2>&1
check "no-pending: dev normal com versao_4_0_0 'proposto' pendurada BLOQUEIA" block "$(run_pending "$d")"
rm -rf "$d"
...
(
    cd "$d"
    printf 'versao_4_0_0\n' > .hbn/active-version
    printf -- '---\ntitulo: boot v4\nstatus: ativo\n---\n# BOOT\n' > versao_4_0_0/BOOT.md
    git add .hbn/active-version versao_4_0_0/BOOT.md
) >/dev/null 2>&1
check "no-pending: ativacao atomica (flip + BOOT ativo) resolve e passa" pass "$(run_pending "$d")"
rm -rf "$d"
...
(
    cd "$d"
    printf -- '---\ntitulo: boot v4\nstatus: congelado\ntemperatura: glacier\n---\n# BOOT\n' > versao_4_0_0/BOOT.md
    git add versao_4_0_0/BOOT.md
) >/dev/null 2>&1
check "no-pending: congelamento da proposta resolve e passa" pass "$(run_pending "$d")"
rm -rf "$d"

d="$(make_pending_repo proposto)"
( cd "$d" && printf 'dev\n' >> versao_3_0_0/base.md && git add versao_3_0_0/base.md ) >/dev/null 2>&1
check "no-pending: versao ATIVA em status proposto e estado invalido (BLOQUEIA)" block "$(run_pending "$d")"
rm -rf "$d"
