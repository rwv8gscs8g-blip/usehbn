...
if [[ -z "$ATRIB" ]]; then
    ...
    case "$(hbn_context_dep_state "$STATE_REPO_PATH")" in
        ATIVO)
            ;;
        NOOP)
            guard_ok "STATE ausente no indice/HEAD/base: contexto de genese/pre-instalacao; G-FAM sem alvo. Instale o contexto com scripts/hbn-install-guard antes de ativar este guard."
            exit 0
            ;;
        DISARM|*)
            guard_fail "${STATE_PATH} existia no HEAD/base e foi removido neste commit/range. G-FAM nao vira no-op por desarme; restaure o STATE ou trate a remocao fora deste commit."
            exit 1
            ;;
    esac
    SC="$(git show "$(hbn_context_current_ref "$STATE_REPO_PATH")" 2>/dev/null || true)"
    if [[ -z "$SC" ]]; then
        guard_fail "STATE indisponivel no blob staged/HEAD — G-FAM nao pode validar atribuicao. Erro de instalacao/rito; stageie .hbn/relay/STATE.md valido antes de ativar este guard."
        exit 1
    fi
    STATE_TMP="$(mktemp)"
    ...
    if [[ "$ASSIGNMENT_STATUS" != "ok" ]]; then
        ...
        guard_fail "Nao foi possivel parsear atribuicao do STATE staged (${ASSIGNMENT_STATUS:-sem-status}). ${ASSIGNMENT_ERROR:-Campo atribuicao ausente.} G-FAM exige atribuicao YAML aninhada valida."
        exit 1
    fi
    eval "$ASSIGNMENT_KV"
    IMPL_R="${implementador:-}"
    AUD_R="${auditores:-}"
    if [[ -z "$IMPL_R" || -z "$AUD_R" ]]; then
        guard_fail "Atribuicao do STATE staged sem implementador ou auditores. G-FAM nao pode validar anti-groupthink em vacuo."
        exit 1
    fi
    for a in $AUD_R; do
        if [[ "$a" == "$IMPL_R" ]]; then
