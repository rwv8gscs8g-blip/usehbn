...
`usehbn start` é a cerimônia de entrada de um ciclo de ondas — fast_track
...
1. BLOQUEADOR: artefato de escritor paralelo cujo nome não casa
   ...
   `escrita_paralela` (apelidos de perfis ADR-015).
2. BLOQUEADOR: linha nova do REGISTRY (staged — `git show :REGISTRY.md`)
   ...
   divergente do `created_at` da mesma linha.
