...
## §7 Campo `proximo_ponto` — checkpoint mecanico do proximo ato

O STATE declara exatamente um bloco top-level `proximo_ponto:` quando a proxima
acao precisa ser checada por maquina. O bloco tem seis campos obrigatorios:

```
proximo_ponto:
  passo: "<descricao atomica do passo>"
  ato: implementacao|cross-audit|hearback|selagem|freeze|fim
  destino: "<apelido canonico do mapa de familias ou human>"
  gate: nenhum|hearback_humano
  bloco_ref: "<path versionado existente ou nenhum>"
  status: pendente|em_curso|concluido
```

Regras mecanicas:

1. `passo` e string nao-vazia.
2. `destino` deve existir no mapa canonico de familias usado pelos guards, com
   `human` e `nenhum` como destinos reservados.
3. `bloco_ref` deve existir no indice/HEAD, exceto quando `ato=fim`; nesse caso
   o unico valor valido e `nenhum`.
4. Campo duplicado, campo extra ou `proximo_ponto` inline bloqueia.
5. O guard que enforca esta secao e `guards/assert-next-checkpoint.sh`; ele le
   somente blob staged/HEAD e nunca a working tree.
