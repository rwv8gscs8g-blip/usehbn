---
titulo: "Prompt cross-audit fix-gexc-sigpipe (0083) — antigravity/Google"
tipo: prompt
status: congelado
temperatura: glacier
path: .hbn/messages/20260621-074500-opus-4-8-prompt-cross-audit-fix-gexc-0083-antigravity.md
...
# Prompt cross-audit fix-gexc-sigpipe (0083) — antigravity
...
2. REPRODUZA o bug e o fix:
   - Forma antiga (em fixture/manual): `set -o pipefail; git show :.hbn/relay/STATE.md | grep -E '^[[:space:]]*-' | grep '🔴' | grep -qiE 'exce'; echo $?` => 141 (falso-negativo).
   - Forma nova (no guard atual, ~linha 126): usa `grep -ciE 'exce'` (le toda a entrada) => sem SIGPIPE.
3. Leia guards/assert-exception-traceable.sh: confirme que as checagens do sinal (d) (🔴 de excecao + PROPOSED_UNTIL_CROSS_AUDIT) NAO terminam mais um pipe de git show em `grep -q`; confirme que a SEMANTICA e a MESMA (mesmos criterios — nao afrouxou: sem 🔴 de excecao ainda bloqueia; sem PROPOSED_UNTIL_CROSS_AUDIT ainda bloqueia). Cite linhas.
4. bash guards/hbn-guards-runner.sh -> "Todos os guards passaram". Rode 3x e confirme DETERMINISTICO (exit 0 sempre) — o bug era um race.
5. bash guards/tests/run-guard-tests.sh -> 0 falharam (note o caso de regressao G-EXC com STATE grande).
6. bash guards/tests/adversarial-battery.sh -> BATERIA VERDE.
7. readback 0083: status implemented_pending_cross_audit; PROPOSED_UNTIL_CROSS_AUDIT; escopo files_allowed respeitado; guards/hbn-guards-runner.sh, guards/data, read-list, orchestrator-profile-spec, workflow, ci-battery NAO tocados.
8. Burla a procurar: o fix AFROUXOU o sinal (d)? (ex.: agora passa mesmo SEM 🔴 de excecao, ou conta linha errada). Construa fixture sem 🔴 de excecao e confirme que AINDA BLOQUEIA. Se o fix mudou a semantica ou deixou passar caso ruim, vote NAO.

ENTREGUE: crie .hbn/results/20260621-080000-antigravity-cross-ia-fix-gexc-sigpipe-0083.md (UNTRACKED — NAO comite) com EXATAMENTE:
---
