...
#     - path: REGISTRY.md
#       install: skeleton
#       source: guards/fixtures/registry-skeleton.md
...
ASSIGNMENT_KV="$(hbn_extract_assignment_kv "$STATE_TMP")"
ASSIGNMENT_STATUS="$(printf '%s\n' "$ASSIGNMENT_KV" | awk -F= '$1=="status"{print $2; exit}')"
if [[ "$ASSIGNMENT_STATUS" != "ok" ]]; then
    ASSIGNMENT_ERROR="$(printf '%s\n' "$ASSIGNMENT_KV" | awk -F= '$1=="error"{print substr($0, index($0,$2)); exit}')"
    guard_fail "Nao foi possivel parsear atribuicao do STATE staged (${ASSIGNMENT_STATUS:-sem-status}). ${ASSIGNMENT_ERROR:-Campo atribuicao ausente.} G-NUM exige atribuicao YAML aninhada valida; rode o rito/instalador antes de criar artefatos de evento."
    exit 1
fi
eval "$ASSIGNMENT_KV"
PARALELO="${escrita_paralela:-}"
...
KNOWN="${PARALELO:-} ${orquestrador:-} ${implementador:-} ${auditores:-}"
if [[ -d "$MODELS_DIR" ]]; then
    for p in "$MODELS_DIR"/*.json; do
        [[ -e "$p" ]] || continue
        alias="$(basename "$p" .json)"
        if ! python3 - "$p" "$alias" >/dev/null 2>&1 <<'PY'
import json, sys
path, alias = sys.argv[1:3]
data = json.load(open(path))
if data.get("apelido", alias) != alias:
    raise SystemExit(1)
if not (data.get("fornecedor") or data.get("familia")):
    raise SystemExit(1)
PY
        then
            guard_fail "Perfil de modelo invalido para '${alias}' em ${p}. O perfil deve ser JSON e declarar apelido coerente + fornecedor/familia; arquivo vazio nao e perfil normativo."
            exit 1
        fi
        KNOWN="$KNOWN $alias"
    done
...
if [[ -n "$ADDED" ]]; then
    while IFS= read -r f; do
        ...
        if [[ "$MODELS_PRESENT" -ne 1 ]]; then
            guard_fail "Artefato NOVO em serie de evento '${f}' mas ${MODELS_DIR} esta ausente. Erro de INSTALACAO: rode hbn-install-guard para criar .hbn/models/<agente>.json minimo por agente da atribuicao."
            FAIL=1
            continue
        fi
        missing_profiles=0
        for ag in ${orquestrador:-} ${implementador:-} ${auditores:-} ${PARALELO:-}; do
            [[ -z "$ag" ]] && continue
            if [[ ! -f "${MODELS_DIR}/${ag}.json" ]]; then
                guard_fail "Atribuicao do STATE referencia agente '${ag}', mas falta ${MODELS_DIR}/${ag}.json. Rode hbn-install-guard --guard assert-parallel-id para instalar perfil minimo validavel."
                missing_profiles=1
            fi
        done
        if [[ "$missing_profiles" -ne 0 ]]; then
            FAIL=1
            continue
        fi
        base="$(basename "$f")"
