...
#       install: refuse
#   dirs: []
#   state_fields: [atribuicao, sinais_abertos]
#   guards: [assert-role-family]
#   env: []
# ---HBN-REQUIRES-END---
set -euo pipefail
...
case "$(hbn_context_dep_state "$STATE_REPO_PATH")" in
    ATIVO)
        ;;
    NOOP)
        guard_ok "STATE ausente no indice/HEAD/base: contexto de genese/pre-instalacao; G-EXC sem alvo. Instale o contexto com scripts/hbn-install-guard antes de ativar este guard."
        exit 0
        ;;
    DISARM|*)
        guard_fail "${STATE_PATH} existia no HEAD/base e foi removido neste commit/range. G-EXC nao vira no-op por desarme; restaure o STATE ou trate a remocao fora deste commit."
        exit 1
        ;;
esac

blob_ref() {
    hbn_context_current_ref "$1"
}
...
if [[ "$ASSIGNMENT_STATUS" != "ok" ]]; then
    ...
    guard_fail "Nao foi possivel parsear atribuicao do STATE staged/HEAD (${ASSIGNMENT_STATUS:-sem-status}). ${ASSIGNMENT_ERROR:-Campo atribuicao ausente.} G-EXC exige implementador checavel; vacuo nao e excecao rastreavel."
    exit 1
fi
eval "$ASSIGNMENT_KV"
IMPL="${implementador:-}"
...
if [[ -z "$MSG_FILE" ]] && ! hbn_ci_range_mode; then
...
TRAILERS_VERIFIED=""
if [[ -n "$MSG_FILE" ]] || hbn_ci_range_mode; then
    TRAILERS_VERIFIED=", trailers verificados"
fi
guard_ok "Exceção F-01 RASTREÁVEL: authorization no readback, sinais no STATE${TRAILERS_VERIFIED} — PROPOSED_UNTIL_CROSS_AUDIT vigente."
exit 0
