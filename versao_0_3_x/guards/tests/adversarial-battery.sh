...
try_burla "B23 CI trailer token sem autorizacao humana" "G-EXC" "$( ( cd "$d" && HBN_CI=1 HBN_DIFF_BASE="$base" bash "$GUARDS_DIR/assert-exception-traceable.sh" >/dev/null 2>&1 ); echo $? )"
rm -rf "$d"
