...
registry_added() {
    if hbn_ci_range_mode; then
        git diff --name-only --diff-filter=A "${HBN_DIFF_BASE}...HEAD" -- "$REGISTRY_REPO_PATH" 2>/dev/null | grep -qxF "$REGISTRY_REPO_PATH"
    else
        git diff --cached --name-only --diff-filter=A -- "$REGISTRY_REPO_PATH" 2>/dev/null | grep -qxF "$REGISTRY_REPO_PATH"
    fi
}

GENESIS_REGISTRY=0
if registry_added; then
    GENESIS_REGISTRY=1
fi

if [[ "$GENESIS_REGISTRY" -ne 1 && ! registry_changed ]]; then
    if ! git show "$(blob_ref "$REGISTRY_REPO_PATH")" >/dev/null 2>&1; then
        guard_fail "REGISTRY.md ausente na raiz — erro de INSTALACAO, nao violacao de linha. Rode: hbn-install-guard --guard assert-arvore-label --target <repo> (cria esqueleto byte-a-byte de guards/fixtures/registry-skeleton.md). ADR-011 D4."
        exit 1
    fi
...
if [[ "$GENESIS_REGISTRY" -ne 1 ]] && ! git show "$(base_blob_ref "$REGISTRY_REPO_PATH")" >/dev/null 2>&1; then
...
registry_added_lines() {
    if [[ "$GENESIS_REGISTRY" -eq 1 ]]; then
        git show "$(blob_ref "$REGISTRY_REPO_PATH")" 2>/dev/null | grep -E '^\|' || true
        return 0
    fi
    if hbn_ci_range_mode; then
