# PROPOSTA — texto corrigido do AGENTS.md (não-normativo até aplicado)

> Zona livre / untracked (knowledge 0024). Este documento **não** substitui o
> `AGENTS.md` rastreado; é uma proposta de curadoria P0 a ser revisada e
> promovida por onda formal (aprovação humana → cross-audit ≠-família → aplicação).
> Truth Barrier: cada correção abaixo cita o arquivo:linha do erro original.

## Correções vs disco

O `AGENTS.md` atual (raiz do repo) contém afirmações que **contradizem o disco**.
Verificação: `ls -d modules radar 2>&1` → ambos retornam "No such file or
directory"; `git ls-files | sed 's#/.*##' | sort | uniq -c` lista as partições
reais (core, methodology, docs, agents, guards, schemas, src, tests, reports,
auditoria, inbox, examples, .hbn, …) — nenhuma `modules/` nem `radar/`.

| # | Erro no AGENTS.md atual | Linha(s) | Por que é falso (evidência no disco) | Correção proposta |
|---|---|---|---|---|
| 1 | "Protocol Specification … `modules/` (per ADR-003 — populated progressively…)" | AGENTS.md:26 | `modules/` **não existe** (`ls -d modules` → No such file or directory). As specs normativas vivas moram em `core/` (`core/protocol.md`, `core/esteira-pre-transicao.md`, `core/exuvia-fitness-criteria.md`, etc.). | Apontar Protocol Specification para `core/` (specs vivas seladas). |
| 2 | Bloco de estrutura de topo lista `modules/` como "normative protocol specification" | AGENTS.md:64 | Pasta inexistente; ponteiro morto na própria porta de entrada (viola P2). | Remover `modules/`; declarar `core/` como casa das specs. |
| 3 | "`radar/` technology phagocytosis tracking (standalone partition)" | AGENTS.md:67 | `radar/` **não existe** (`ls -d radar` → No such file or directory). O tracking de fagocitose vive em `docs/PHAGOCYTOSIS.md`. | Remover `radar/`; apontar fagocitose para `docs/PHAGOCYTOSIS.md`. |
| 4 | "`core/` legacy partition; superseded by modules/ progressively (per ADR-003 §B)" | AGENTS.md:69 | **Falso e perigoso**: `core/` é a casa das specs normativas VIVAS e seladas (`status: accepted`), incluindo `core/esteira-pre-transicao.md` (spec-core accepted), `core/exuvia-fitness-criteria.md` (accepted), `core/role-cards.md`, `core/protocol.md`. Não é legado e não foi superado por `modules/` (que não existe). | Declarar `core/` como partição VIVA das specs normativas. |
| 5 | "`docs/` legacy partition; pieces being migrated to modules/ + methodology/" | AGENTS.md:70 | `docs/` está ativo: contém `docs/PHAGOCYTOSIS.md`, `docs/ARCHITECTURE.md`, `docs/WAVE-PLAN-V0.3.0.md`, a zona livre `docs/brainstorm/**` e o glossário-alvo. Migrar "para modules/" é impossível (não existe). | Descrever `docs/` como documentação de explicação/arquitetura + zona livre, sem "legacy/superseded por modules/". |
| 6 | "Reference Implementation … To be built in `examples/` (future)" | AGENTS.md:25 | Parcialmente verdadeiro: `examples/` existe mas tem só 2 arquivos rastreados (scaffold). | Manter como "future/scaffold" — já está honesto; preservado. |

O que já está **correto** no AGENTS.md atual e é preservado: o ponteiro de
maturidade para `methodology/MATURITY-MATRIX.md` (AGENTS.md:17 — confirmado fonte
única); os 13 princípios em `methodology/PRINCIPIOS-CONSTITUCIONAIS.md`
(AGENTS.md:29-52); a tabela de sinais HBN (AGENTS.md:74-96); as convenções de
comunicação e Truth Barrier (AGENTS.md:98-105); a delegação para `agents/`
(AGENTS.md:107-119); o bloco `useHBN-version` declarativo (AGENTS.md:121-148);
stack Python/pytest/schemas (AGENTS.md:54-59). Tudo isso é mantido.
