...
|---|---|---|---|---|
| **CLI** | Implementado | `src/usehbn/cli.py`; ~17 subcomandos roteados em `main()`; integracao com Readback, Hearback, ERP, Relay, Handoff, Connector. | God-object (~1700 LOC). | Refatoracao interna (Onda 7) sem mudar contrato externo. |
...
| **Guardian** | Parcial (advisory) | `src/usehbn/protocol/guardian.py`; 2 checks; loga em `logs/guardian.jsonl`. | Idem Truth Barrier. | RFC-0001 + modo `--enforce` opt-in (roadmap). |
